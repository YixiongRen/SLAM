...
class Particle:
    ...
    def initialize_new_landmark(self, measurement_in_scanner_system,
        ...
        self.landmark_positions.append(np.array([landmark_x,landmark_y]))  # Replace this.
    ...
    def update_particle(self, measurement, measurement_in_scanner_system,
        ...
        # Else update the particle's EKF for the corresponding particle.
            ...
            w, index = max(likelihoods), likelihoods.index(max(likelihoods))  # Replace this by a search for max, argmax.
    ...
    def decrement_visible_landmark_counters(self, scanner_displacement):
        ...
        for i in xrange(len(self.landmark_counters)):
            r,theta = self.h_expected_measurement_for_landmark(i,scanner_displacement)[0]
            if theta>= min_bearing and theta<= max_bearing:
    ...
    def remove_spurious_landmarks(self):
        ...
        for i in xrange(len(self.landmark_counters)):
            if self.landmark_counters[i]>=0:
                posi.append(self.landmark_positions[i])
                cov.append(self.landmark_covariances[i])
                count.append(self.landmark_counters[i])
        self.landmark_positions = posi
        self.landmark_covariances = cov
        self.landmark_counters = count
        # --->>> Insert your new code here.
