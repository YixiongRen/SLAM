...
class ExtendedKalmanFilterSLAM:
    ...
    def predict(self, control):
        ...
        G = eye(len(G3[0])+self.number_of_landmarks*2)
        for i in xrange(len(G3[0])):
            for j in xrange(len(G3[0])):
                G[i][j] = G3[i][j]

        n = len(R3[0])+self.number_of_landmarks*2
        R = zeros([n,n])
        for i in xrange(len(R3[0])):
            for j in xrange(len(R3[0])):
                R[i][j] = R3[i][j]
    ...
    def add_landmark_to_state(self, initial_coords):
        ...
        l = len(self.state)
        state=zeros(l+2,float)
        state[0:l] = self.state
        
        state[l] = initial_coords[0]
        state[l+1] = initial_coords[1]

        self.state = state

        self.number_of_landmarks += 1
        covariance = eye(len(self.covariance[0])+2)
        for i in xrange(len(covariance[0])):
            covariance[i][i] = 1e10

        for i in xrange(len(self.covariance[0])):
            for j in xrange(len(self.covariance[0])):
...
if __name__ == '__main__':
    ...
    for i in xrange(len(logfile.motor_ticks)):
        ...
        kf.predict(control)
        #cylinder不动，自身运动，更新state并扩充covariance
        
        # Correction.
        #在get_observation方法中，将scan_data[i]求出的cylinder坐标
        #与kf.state里已有的cylinder坐标相比较，得出匹配的cylinder对
        #如若kf.state里暂时还没有cylinder，则返回cylinder_index = -1
