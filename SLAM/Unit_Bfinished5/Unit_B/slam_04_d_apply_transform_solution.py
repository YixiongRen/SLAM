...
def find_cylinder_pairs(cylinders, reference_cylinders, max_radius):
    cylinder_pairs = []
    for r in xrange(len(cylinders)):       
        i = 0
        j = 0
        Dist = 1000000000
        EulerDist = 0
        for t in xrange(len(reference_cylinders)):
            EulerDist = ((cylinders[r][0]-reference_cylinders[t][0])**2+(cylinders[r][1]-reference_cylinders[t][1])**2)**0.5
            if EulerDist < Dist:
                Dist = EulerDist
                i = r
                j = t
...
##            print left_list
        for i in xrange(len(left_list)):
            ...
            lx_ = left_list[i][0]-lc[0]
            ly_ = left_list[i][1]-lc[1]
            rx_ = right_list[i][0]-rc[0]
            ry_ = right_list[i][1]-rc[1]
            cs += rx_*lx_ + ry_*ly_
            ss += (-1)*rx_*ly_ + ry_*lx_
...
def correct_pose(pose, trafo):
    ...
    pose0 = pose[0]*c-s*pose[1]+tx
    pose1 = pose[0]*s+c*pose[1]+ty
    pose2 = pose[2] + atan2(s,c)
    
    return (pose0, pose1, pose2)  # Replace this by the corrected pose.
