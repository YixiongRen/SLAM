...
def filter_step(old_pose, motor_ticks, ticks_to_mm, robot_width,
    ...
    if motor_ticks[0] == motor_ticks[1]:
        ...
        # scanner displacement? [y/n]: n
        
        return (x, y, theta)

    else:
        # Turn. Compute alpha, R, etc.
        body_x = old_pose[0]- scanner_displacement*cos(old_pose[2])
        body_y = old_pose[1]- scanner_displacement*sin(old_pose[2])
        alpha = (motor_ticks[1]-motor_ticks[0])*ticks_to_mm/robot_width
        R = motor_ticks[0]*ticks_to_mm/alpha
        Cx = body_x- (R+robot_width/2)*sin(old_pose[2])
        Cy = body_y+ (R+robot_width/2)*cos(old_pose[2])
        theta = (old_pose[2] + alpha)%(2*pi)
        body_x = Cx + (R+robot_width/2)*sin(theta)
        body_y = Cy - (R+robot_width/2)*cos(theta)
        x = body_x+ scanner_displacement*cos(old_pose[2])
        y = body_y+ scanner_displacement*sin(old_pose[2])
